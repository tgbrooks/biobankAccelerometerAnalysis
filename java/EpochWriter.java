...
public class EpochWriter {
  ...
  private double[] prevXYZTL = { -1, -1, -1, -1, -1}; // x, y, z, temp, and light at prevTimeVal
...
//      System.out.println(DF6.format(Double.POSITIVE_INFINITY));
  ...
  public boolean newValues(
      ...
      double x, double y, double z, double temperature, double light,
      int[] errCounter) {
    ...
    if (time-prevTimeVal >= epochPeriod * 2 * 1000 && prevTimeVal != UNUSED_DATE) {
      ...
      if (timeVals.size()>minSamplesForEpoch) {
        writeEpochSummary(millisToTimestamp(epochStartTime), timeVals, xVals, yVals, zVals, temperatureVals, lightVals, errCounter);
      } else {
   ...
   * The method:
   *  -resamples all data (x/y/z/time/temperatureVals/lightVals) to the intendedSampleRate
  ...
  private void writeEpochSummary(LocalDateTime epochStartTime,
      ...
      List<Double> xVals, List<Double> yVals, List<Double> zVals, List<Double> temperatureVals, List<Double> lightVals,
      int[] errCounter) {
    ...
    epochSummary += "," + DF2.format(mean(lightVals));
